...
public class NoteService {
    ...
    public List<Note> findNotesByText(String query) {
        ...
        return notes.values().stream().filter(note -> note.getTitle().toLowerCase().contains(lowerQuery) || note.getText().toLowerCase().contains(lowerQuery)).collect(Collectors.toList());
    }
    ...
    public List<Note> findNotesByTags(Set<String> searchTags) {
        ...
        Set<String> normalizedTags = searchTags.stream().map(String::toLowerCase).collect(Collectors.toSet());

        return notes.values().stream().filter(note -> note.getTags().containsAll(normalizedTags)).collect(Collectors.toList());
    }

    public Set<String> getAllTags() {
        return notes.values().stream().flatMap(note -> note.getTags().stream()).collect(Collectors.toSet());
    }
